...
# Time spent: start-07.11.2021; finish-15.11.2021-9 days
...
def get_guessed_word(secret_word, letters_guessed):
    ...
    for letter in secret_word:
      if letter not in letters_guessed:
        guessed_word[guessed_word.index(letter)] = '_ '
    string_1 = ''.join(guessed_word)
    return string_1
...
def get_available_letters(letters_guessed):
    ...
    string_2 = ''.join(available_letters)
    return string_2
    ...
    while not is_word_guessed(secret_word, letters_guessed) and guesses_remaining > 0:
      ...
      if not letter.isalpha() or len(letter) != 1:
        ...
        if letter in letters_guessed:
          if warnings_remaining > 0:
            warnings_remaining -= 1
            print('Oops! You\'ve already guessed that letter. You now have ' + str(warnings_remaining) + ' warnings left:' + get_guessed_word(secret_word, letters_guessed))
          else:
            guesses_remaining -= 1
          ...
          if letter in secret_word:
            ...
            if is_word_guessed(secret_word, letters_guessed):
              print('------------')
              print('Congratulations, you won! Your total score for this game is: ' + str(guesses_remaining * len(result)))
          else:
            if letter in 'aeiou':
              guesses_remaining -= 2
            else:
...
def match_with_gaps(my_word, other_word):
    ...
    returns: boolean, True if all the actual letters of my_word match the 
        ...
        False otherwise: 
    '''
    space = my_word.replace(' ','')
    letters = ''
    if len(space) != len(other_word):
      return False
    for i in range(len(space)):
      if space[i] != '_':
        if space[i] != other_word[i]:
          return False
          letters += other_word[i]
      else:
